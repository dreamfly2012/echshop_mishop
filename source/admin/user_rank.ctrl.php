	...
	*/
	...
	class user_rankControl extends skymvc{
		...
		public function __construct(){
			parent::__construct();
			$this->loadmodel(array("user_rank"));
		}
		...
		public function onDefault(){
			...
			$rscount=true;
			$data=$this->user_rank->select($option,$rscount);
		...
		public function onAdd(){
			$id=get_post("id","i");
			if($id){
				$data=$this->user_rank->selectRow(array("where"=>"id={$id}"));
		...
		public function onSave(){
			...
			if($id){
				$this->user_rank->update($data,"id='$id'");
			}else{
				$this->user_rank->insert($data);
			}
			$this->goall("保存成功");
		}
