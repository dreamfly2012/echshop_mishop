...
class seoControl extends skymvc{
	...
	public function onDefault(){
		...
		$rscount=true;
		$data=$this->seo->select($option,$rscount);
	...
	public function onAdd(){
		...
		$id=get_post('id','i');	
		if($id){
			$seo=$this->seo->getRow("SELECT * FROM ".table('seo')." WHERE id='$id'  ");	
	...
	public function onSave(){
		...
		if($id){
			$this->seo->update($data,"   id='$id' ");
		}else{
			$this->seo->insert($data);
		}
		$this->goall("操作成功");	
	}
	...
	public function onDel(){
		$id=get_post('id','i');
		if($this->seo->delete(array("id"=>$id))){
