<?php
	...
	class pmControl extends skymvc{
		...
		public function __construct(){
			parent::__construct();
			$this->loadmodel(array("pm","user","login"));
		}
		...
		public function onDefault(){
			...
			$rscount=true;
			$data=$this->pm->select($option,$rscount);
			...
			if($data){
				...
				$user=$this->user->getUserByIds($uids);
		...
		public function onShow(){
			$id=get_post("id","i");
			if($id){
				$data=$this->pm->selectRow(array("where"=>"id={$id}"));
		...
		public function onStatus(){
			...
			$this->pm->update(array("status"=>$status),"id=$id");
