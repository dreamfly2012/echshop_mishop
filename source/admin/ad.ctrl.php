	...
	class adControl extends skymvc{
		...
		public function onAdd(){
			...
			if($id){
				...
				if(get_post('tag_id','i')) $tag=$this->ad_tags->selectRow(array("where"=>"  tag_id=".get_post('tag_id','i').""));
				if(get_post('tag_2nd_id','i')) {
					$tag_2nd=$this->ad_tags->selectRow(array("where"=>"tag_id=".get_post('tag_2nd_id','i').""));
					$this->smarty->assign("tag_list_2nd",$this->ad_tags->tagList(get_post('tag_id','i'),0));
				}
			}
		...
		public function onTag_id_2nd(){
			$tag_id=get('tag_id','i');
			$data=$this->ad_tags->taglist($tag_id,0);
