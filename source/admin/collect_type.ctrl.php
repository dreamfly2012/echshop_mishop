	...
	*/
	...
	class collect_typeControl extends skymvc{
		...
		public function __construct(){
			parent::__construct();
			$this->loadmodel(array("collect_type"));
		}
		...
		public function onDefault(){
			...
			$rscount=true;
			$data=$this->collect_type->select($option,$rscount);
		...
		public function onAdd(){
			...
			if($type_id){
				$data=$this->collect_type->selectRow(array("where"=>"type_id={$type_id}"));
		...
		public function onSave(){
			...
			if($type_id){
				$this->collect_type->update($data,"type_id='$type_id'");
			}else{
				
				$this->collect_type->insert($data);
			}
			$this->goall("保存成功");
		}
		...
		public function onStatus(){
			...
			$this->collect_type->update(array("status"=>$status),"type_id=$type_id");
			exit(json_encode(array("error"=>0,"message"=>"状态修改成功")));
		}
		
		public function onDelete(){
			$type_id=get_post('type_id',"i");
			$this->collect_type->delete("type_id={$type_id}");
