...
class articleControl extends skymvc{
	...
	public function __construct(){
		parent::__construct();
		$this->loadmodel(array("article","category","article_data","attribute_cat","attribute","article_attr","model_index"));	
	...
	public function onDefault(){
		...
		$data=$this->article->select($op,$rscount);
		if($data){
			foreach($data as $k=>$v){
	...
	public function onAdd(){
		...
		if($id){
			...
			$data=$this->article->selectRow(array("where"=>"is_temp=1 AND userid=".DEFAULT_USERID." "));
			if(empty($data)){
				$id=$this->model_index->insert(array("tablename"=>"article"));
				$this->article->insert(array("id"=>$id,"dateline"=>time(),"userid"=>DEFAULT_USERID,"is_temp"=>1,"status"=>99));
	...
	public function onSave(){
		...
		$this->goall($this->lang['add_success'],0,$data,APPADMIN."?m=article&a=add&id=".$id);
	}
	...
	public function onDelete(){
		$id=get('id','i');
		 $this->article->update(array("status"=>99),array("id"=>$id));
	...
	public function onStatus(){
		$id=get('id','i');
		$status=get('status','i');
		$this->article->update(array("status"=>$status),array("id"=>$id));
	...
	public function onRecommend(){
		...
		$this->article->update(array("is_recommend"=>$is_recommend),array("id"=>$id));	
		echo json_encode(array("error"=>0,"message"=>$this->lang['save_success']));	 
	}
	...
	public function onnew(){
		$id=get('id','i');
		$isnew=get('isnew','i');
		$this->article->update(array("isnew"=>$isnew),array("id"=>$id));	
		echo json_encode(array("error"=>0,"message"=>$this->lang['save_success']));	 
	}
	...
	public function onhot(){
		$id=get('id','i');
		$ishot=get('ishot','i');
		$this->article->update(array("ishot"=>$ishot),array("id"=>$id));	
		echo json_encode(array("error"=>0,"message"=>$this->lang['save_success']));	 
	}
	...
	public function onattrByCat(){
	 ...
	 public function onCategory(){
		...
		if($ids){
			foreach($ids as $id){
				$this->article->update(array("catid"=>$catid),"id=".$id);
			}
		}
		$this->goall("修改成功");
	}
