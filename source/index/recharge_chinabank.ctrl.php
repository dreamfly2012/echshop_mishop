		   ...
		   if($v_pstatus=="20")
			{
			   //支付成功
				...
				//------------------------------
						...
						$row=$this->recharge->selectRow(array("where"=>" orderno='".$v_oid."' "));
						...
						if(empty($row) or $row['status']==1){
							...
							$this->recharge->update(array("status"=>1,"pay_orderno"=>$transaction_id)," id='".$row['id']."'" );
							
							//判断订单是否购物订单 如果是处理订单
							if(substr($out_trade_no,0,2)=='o_'){
								$this->loadModel(array("order"));
								$order=$this->order->selectRow(array("orderno='".$out_trade_no."' "));
								$this->loadControl("jfapi");
								//添加金钱消费记录		
								$this->jfapiControl->addMoney(array(
									"money"=>"-".$order['money'],
								...
								$this->order->update(array("ispay"=>1),"order_id=".$order['order_id']);
