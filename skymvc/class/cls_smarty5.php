...
class Smarty
{
   ...
   public function __construct()
    ...
    public function display($filename, $cache_id = '')
    {
		...
		$out=preg_replace("/<title[^>]*>(.*)<\/title>/i","<title>\\1".POWEREDBY."</title>",$out);
		if($this->html_file){
			error_reporting(E_ALL ^ E_NOTICE);
    ...
    public function select($tag)
    {
		...
		*/	
			...
			*/
			...
			//解析数组 {$a}[][][];
			if(isset($module[1])){
				return '<?php C()->loadModulecontrol("'.$module[1].'","'.$m[1].'");$this->assign("'.$k[1].'",$GLOBALS["'.$m[1].'Control"]->'.$fun.'); ?>';
			}else{
				return '<?php C()->loadControl("'.$m[1].'");$this->assign("'.$k[1].'",$GLOBALS["'.$m[1].'Control"]->'.$fun.'); ?>';
			}
